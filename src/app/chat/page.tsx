...
export default function ChatPage() {
  ...
  return (
    /* 🚨 RESPONSIVE FIX:
       - Mobile: Takes full height (100dvh) to cover the screen perfectly.
       - Desktop (sm+): Removes height constraint (h-auto), allowing the global layout (Header/Footer) 
         to dictate the flow, while ChatListScreen handles its own inner sizing.
    */
    <div
      className={clsx(
        "flex flex-col bg-background w-full",
        "h-[100dvh] overflow-hidden",
        "sm:h-auto sm:overflow-visible sm:py-6",
      )}
    >
      <ChatListScreen onNavigateToDetail={handleNavigateToChat} />
    </div>
