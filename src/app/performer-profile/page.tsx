...
//           roles: profile.roles,
//         });
//         toast({ variant: "success", description: "Добавлено в избранное" });
//       }
...
//         description,
//       });
//       toast({ variant: "success", title: "Добавлено" });
//       return true;
//     } catch {
//       return false;
//     }
...
// 🚨 NEW: Types for Hierarchical Categories
...
export default function PerformerProfilePage() {
  ...
  // 🚨 NEW: Store full category objects, not just strings
  ...
  useEffect(() => {
    getSiteSettings()
      .then((settings) => {
        // 🚨 Store the full objects so we can access subCategories in the UI
        if (settings?.siteCategories) {
  ...
  // Standard toggle for sub-categories
  const toggleTempRole = (roleName: string) => {
    setTempSelectedRoles((prev) =>
      prev.includes(roleName)
        ? prev.filter((r) => r !== roleName)
  ...
  return (
    <>
      ...
      {/* 🚨 NEW: Hierarchical Category Edit Dialog */}
      <Dialog
        open={isCategoryDialogOpen}
        onOpenChange={setIsCategoryDialogOpen}
      >
              ...
              adminCategories.map((category) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Sub-categories slide down when main category is selected */}
                    {isCategorySelected &&
                      category.subCategories &&
                      category.subCategories.length > 0 && (
